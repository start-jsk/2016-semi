...
//#include "rcb4.h"           // 20151005 iwaishi
#include "khr_driver/rcb4.h"  // 20151005 iwaishi
...
int kondo_init(KondoRef ki)
{
	return kondo_init_custom(ki, RCB4_BAUD, RCB4_USB_VID, RCB4_USB_PID,
                                 INTERFACE_ANY, 0);  // 20151028 iwaishi
}

// 20151028 iwaishi
int kondo_init_index(KondoRef ki, UINT index)
{
	return kondo_init_custom(ki, RCB4_BAUD, RCB4_USB_VID, RCB4_USB_PID,
                                 INTERFACE_ANY, index);
}
 ...
 * Returns 0 if successful, error code otherwise.
 */
int kondo_init_custom(KondoRef ki, int baud, int vid, int pid, int interface, UINT index)  // 20151028 iwaishi
{
	assert(ki);
	int i;
	ki->debug = 0;
	...
	// open usb device
	if (ftdi_usb_open_desc_index(&ki->ftdic, vid, pid, NULL, NULL, index) < 0)  // 20151028 iwaishi
		kondo_ftdi_error(ki);
...
int kondo_send_ics_pos(KondoRef ki, UCHAR servos[5], UINT frame)
{
	...
	// verify response
	//if (i != 4 || ki->swap[1] != RCB4_CMD_ICS)  // 20151005 iwaishi
        if (ret != 4 || ki->swap[1] != RCB4_CMD_ICS)  // 20151005 iwaishi
		kondo_error(ki, "Response was not valid");
